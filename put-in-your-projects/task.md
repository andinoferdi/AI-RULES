...
## Guide
- Ikuti `git-workflow.md`: mulai dari `[MAIN_BRANCH]`, pull perubahan terbaru, lalu buat branch baru sesuai `git-naming.md`.
...
## Audit
- Mulai dari branch utama jika itu aturan resmi. PR wajib diarahkan ke `[TARGET_PR_BRANCH]`.
- Jika environment demo memakai branch berbeda dari target PR, jangan langsung menyimpulkan target PR salah. Audit perbedaan `[MAIN_BRANCH]`, `[TARGET_PR_BRANCH]`, dan `[BRANCH_DEMO]` dulu.
- Jika staging/demo lebih maju, ambil perubahan relevan dengan cara paling aman sesuai `git-branch-tips.md`.
- Hasil akhir harus tetap bisa naik ke PR `[TARGET_PR_BRANCH]` tanpa merusak workflow, history, atau flow bisnis.
