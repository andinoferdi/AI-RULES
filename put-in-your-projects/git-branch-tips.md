...
Tips praktis menjaga branch dan Pull Request tetap bersih di `[PROJECT_NAME]`, terutama saat harus resolve conflict ke `[STAGING_BRANCH]` yang sering di-remerge/reset. Panduan utama tetap `git-workflow.md` dan `git-naming.md`.

## 1. Mulai branch dari branch utama
Pakai `origin/[MAIN_BRANCH]` sebagai titik mulai agar branch benar-benar dari remote terbaru.

```bash
git fetch origin --prune
git checkout -b [USERNAME]/feat/nama-fitur origin/[MAIN_BRANCH]
```

Local `[MAIN_BRANCH]` boleh dipakai bila sudah dipastikan sama dengan remote:

```bash
git checkout [MAIN_BRANCH]
git pull --ff-only origin [MAIN_BRANCH]
git checkout -b [USERNAME]/feat/nama-fitur
```
...
```bash
git diff [MAIN_BRANCH]..origin/nama-branch-lain -- path/relevan
...
```bash
git checkout -b [USERNAME]/feat/nama-fitur-clean origin/[STAGING_BRANCH]
...
```bash
git push --force-with-lease origin HEAD:[USERNAME]/feat/nama-fitur
```
...
```bash
git checkout [USERNAME]/feat/nama-fitur
git fetch origin --prune
git pull --ff-only origin [USERNAME]/feat/nama-fitur
```

Jika history berbeda karena remote sudah di-force-with-lease, samakan local ke remote hanya setelah working tree bersih:

```bash
git branch -f [USERNAME]/feat/nama-fitur origin/[USERNAME]/feat/nama-fitur
```
